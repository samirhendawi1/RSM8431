...
class Recommender:
    """
    final fit_score =
        we*env_score + wb*budget_score + wg*group_score
        + wllm*llm_similarity_score + llm_boost
    """
    ...
    def recommend(self, user, df: pd.DataFrame,
        ...
        # LLM similarity (based on tags/features/env/type)
        ...
        # Robust ID matching as strings (handles "P-101" etc.)
        ...
        d["fit_score"] = (we*d["env_score"] + wb*d["budget_score"] + wg*d["group_score"]
                          + self.llm_weight*d["llm_similarity_score"] + d["llm_boost"])

        # >>> Ensure property_id is returned <<<
        cols = ["property_id","location","environment","property_type","nightly_price",
                "min_guests","max_guests","features","tags",
                # keep score columns below if you export them or want to inspect:
                # "env_score","budget_score","group_score",
                # "llm_similarity_score","llm_boost",
                "fit_score"]
